...
var buildChart = function(){
    ...
    function sortData(jsonData){
        jsonData.sort(function(a,b){
            return (a[a.length-1].y0+a[a.length-1].y)-(b[b.length-1].y0+b[b.length-1].y)
        });
    ...
    var handleOver = function(d, phases){
        phases.forEach((change) => {
            ...
            if(d!='light'){
                document.querySelectorAll("#unknown").forEach(function(d){
                    d.style.fill = "gray";
                })
            }
        })
    ...
    ntpNights.sort(compare);
    ntpNights.forEach((a,i)=>{
        a.forEach((b)=>{
            b.x=i;
        })
    })
    ...
    ndvNights.sort(compare);
    ndvNights.forEach((a,i)=>{
        a.forEach((b)=>{
            b.x=i;
        })
    })
    ...
    function compare(a,b){
        var aH = (a[a.length-1]['y0']+a[a.length-1]['y']);
        var bH = (b[b.length-1]['y0']+b[b.length-1]['y']);
        return aH-bH;
    };
