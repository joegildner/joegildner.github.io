...
var labs = {
    '161':{1:[10,16],2:[9,16],3:[10,16],4:[9,16]},
    '162':{1:[8,16],2:[8,16],3:[10,16]},
    '164':{1:[8,16],2:[8,16],3:[10,16],6:[10,12]},
    '165':{1:[12,17],2:[8,16],3:[8,10,12,16],4:[14,16],5:[8,10]},
    '167':{2:[10,12],4:[10,12],},
    '420':{1:[10,11],2:[9,11,15,16],3:[10,11,16,17],4:[10,16],5:[10,12]}
}

function initLabs(){
    var labNums = Object.keys(labs);
    var date = new Date(Date());
    var hour = date.getHours();
    var day = date.getDay();
    labNums.forEach((num) => {
        if(labs[num][day] != undefined){
            var free = 1;
            for(var i=0; i<labs[num][day].length; i+=2){
                if(hour>=labs[num][day][i] && hour<labs[num][day][i+1]){
                    buttonBusy(num);
                }
            }
        }
...
function buttonBusy(num){
    $("#"+num).removeClass("btn-outline-success");
    $("#"+num).addClass("btn-outline-danger");
}
...
function searchFunc(OrigRoom){
    if(OrigRoom == undefined){
        OrigRoom = document.getElementById("searchText").value;
    }
